...
function buttonClick() {
  ...
  //checkbox_Q4を取得
  const inputs_mi = [...document.querySelectorAll('input[name="middle"]:checked')];
  const values_mi = inputs_mi.map(input => input.value);


  //checkbox_Q5を取得
  const inputs_ba = [...document.querySelectorAll('input[name="back"]:checked')];
  const values_ba = inputs_ba.map(input => input.value);
  ...
  console.log(values_mi);
  console.log(values_ba);
...
function draw() {
  ...
  //(0,300,500,500)
  //middleの馬番登録
  const values_mi = [8,9,10,11,12,13];
  //miの個数判定
  ...
  //(0,500,700,500)
  //backの馬番登録
  const values_ba = [14,15,16,17,18];
  //baの個数判定
